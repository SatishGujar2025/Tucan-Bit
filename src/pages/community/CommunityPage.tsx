  ...
  return (
    <div className="min-h-screen bg-black">
      ...
      <header className="lg:hidden fixed top-0 left-0 right-0 z-30 bg-gray-900/80 backdrop-blur-sm p-4 flex justify-between items-center border-b border-gray-700">
        ...
        <div className="absolute inset-0 flex items-center justify-center pointer-events-none z-30">
          <div className="flex items-center space-x-2">
            ...
            <h1 className="text-xl font-bold text-white"><span className="text-[#36CFC9]">Tucan</span><span className="text-[#36CFC9]">Bit</span></h1>
          </div>
        </div>
      ...
      <main className="pt-16 lg:pt-0">
        {/* Hero Section */}
        <section className="relative overflow-hidden h-[50vh] sm:h-[60vh] min-h-[400px] sm:min-h-[500px] flex items-center">
          ...
          <div className="relative z-10 p-4 sm:p-6">
            <div className="max-w-4xl mx-auto">
              ...
              <div className="flex flex-wrap gap-3 sm:gap-4">
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 sm:p-4 border border-white/20 flex-1 min-w-[120px]">
                  <div className="text-xl sm:text-2xl font-bold text-white">{communityStats.totalMembers.toLocaleString()}</div>
                  <div className="text-xs sm:text-sm text-gray-300">Total Members</div>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 sm:p-4 border border-white/20 flex-1 min-w-[120px]">
                  <div className="text-xl sm:text-2xl font-bold text-white">{communityStats.onlineMembers.toLocaleString()}</div>
                  <div className="text-xs sm:text-sm text-gray-300">Online Now</div>
                </div>
                <div className="bg-white/10 backdrop-blur-sm rounded-lg p-3 sm:p-4 border border-white/20 flex-1 min-w-[120px]">
                  <div className="text-xl sm:text-2xl font-bold text-white">{communityStats.totalPosts.toLocaleString()}</div>
            ...
            {activeTab === 'chat' && (
              <div className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
                <div className="flex flex-col lg:flex-row h-[500px] lg:h-[600px]">
                  {/* Channels Sidebar */}
                  <div className="w-full lg:w-64 bg-gray-900 border-b lg:border-b-0 lg:border-r border-gray-700 flex flex-col">
                      ...
                      {chatChannels.map((channel) => (
                        <button
                          ...
                          className={`w-full p-3 flex items-center justify-between hover:bg-gray-800 transition-colors ${
                            selectedChannel === channel.id ? 'bg-blue-600/20 lg:border-r-2 border-blue-500' : ''
                          }`}
                        >
                          ...
                          <div className="flex items-center space-x-1 sm:space-x-2">
                            {channel.unread > 0 && (
                              <span className="bg-red-500 text-white text-xs px-1 sm:px-2 py-1 rounded-full">
                                {channel.unread}
                              </span>
                            )}
                            <span className="text-xs text-gray-500 hidden sm:block">{channel.members}</span>
                          </div>
                        </button>
                      ))}
                    </div>
                      ...
                      {chatMessages.map((msg) => (
                        <div key={msg.id} className="flex items-start space-x-2 sm:space-x-3">
                          <div className="flex-shrink-0">
                            <div className={`w-8 h-8 sm:w-10 sm:h-10 rounded-full flex items-center justify-center text-base sm:text-lg relative ${
                              msg.isStaff ? 'bg-blue-600' : 'bg-gray-700'
                            }`}>
                              {msg.avatar}
                              {msg.isOnline && (
                                <div className="absolute -bottom-1 -right-1 w-2 h-2 sm:w-3 sm:h-3 bg-green-500 rounded-full border-2 border-gray-800"></div>
                              )}
                            </div>
                          </div>
                          <div className="flex-1 min-w-0">
                            <div className="flex items-center space-x-1 sm:space-x-2 mb-1">
                              ...
                              {msg.isStaff && (
                                <span className="px-1 sm:px-2 py-0.5 sm:py-1 bg-blue-600 text-white text-xs rounded-full">Staff</span>
                              )}
                              <span className="text-xs text-gray-400">{msg.timestamp}</span>
                            </div>
                            <p className="text-gray-300 text-sm sm:text-base break-words">{msg.message}</p>
                          </div>
                        </div>
                      ))}
                    </div>
                      ...
                      <div className="flex items-center space-x-2 sm:space-x-3">
                        <div className="flex-1 relative">
                          <input
                            ...
                            className="w-full pl-3 sm:pl-4 pr-10 sm:pr-12 py-2 sm:py-3 bg-gray-700 border border-gray-600 rounded-lg text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-blue-500 text-sm sm:text-base"
                          />
                        ...
                        <button className="bg-blue-600 hover:bg-blue-700 text-white p-2 sm:p-3 rounded-lg transition-colors flex-shrink-0">
