...
const RoulettePage: React.FC = () => {
  ...
  const providers = ['all', ...Array.from(new Set(allRouletteGames.map(game => game.provider)))];
  const themes = ['all', ...Array.from(new Set(allRouletteGames.map(game => game.theme)))];
        ...
        <div className="">
          ...
          <div className="relative z-10 text-center">
            <div className="inline-flex items-center space-x-2 bg-gradient-to-r from-yellow-500/20 to-orange-500/20 border border-yellow-500/30 rounded-full px-6 py-2 mb-6">
              ...
              <span className="text-yellow-400 font-semibold">{allRouletteGames.length}+ Roulette Games Available</span>
              <Zap className="w-5 h-5 text-yellow-400" />
            </div>
            <h1 className="text-6xl md:text-8xl font-bold text-white mb-6 leading-tight">
              <span className="bg-clip-text text-transparent bg-gradient-to-r from-yellow-400 via-orange-500 to-pink-500">RouletteS PARADISE</span>
            </h1>
            <p className="text-xl md:text-2xl text-gray-300 mb-8 max-w-4xl mx-auto leading-relaxed">
              Spin your way to fortune! Discover the most exciting Roulette games with massive jackpots, free spins, and bonus features.
            </p>
        ...
        <div>
          <div className="flex flex-col lg:flex-row gap-4 items-center justify-between">
            <div className="relative flex-1 max-w-md">
              ...
              <input type="text" placeholder="Search roulette games, providers, variants..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-10 pr-4 py-3 bg-gray-700 border border-gray-600 rounded-xl text-white placeholder-gray-400 focus:outline-none focus:border-yellow-500 transition-colors" />
            </div>
            <div className="flex items-center space-x-4">
              ...
              <div className="text-gray-400"><span className="font-semibold text-white">{filteredGames.length}</span> roulette games found</div>
            </div>
          </div>
        </div>
      </section>
