...
const GameTabs: React.FC<GameTabsProps> = ({ title = "Games", showViewAll = true }) => {
  ...
  return (
    <div className="mb-0">
      ...
      {/* Games Grid */}
      <div className="flex overflow-x-auto pb-0 md:grid md:grid-cols-4 lg:grid-cols-6 xl:grid-cols-8 md:overflow-x-visible scrollbar-hide" style={{ gap: '1rem' }}>
        {activeTabData?.games.map((game) => (
          <div key={game.id} className="flex-shrink-0 w-48 md:w-auto mr-4 md:mr-0">
            <GameCard game={game} />
          </div>
        ))}
      </div>
