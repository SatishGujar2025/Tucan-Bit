...
// Modals and some icons are still needed from Lucide for sub-menus and the header
...
const Layout: React.FC = () => {
  ...
  if (isAuthenticated) {
    headerNavItems.push({ id: 'profile', path: '/profile', label: 'Profile', icon: User });
  }
    ...
    <div className="min-h-screen bg-gradient-to-br from-gray-900 via-[#3C1A4F] to-[#000000]">
      ...
      <div className={`fixed inset-y-0 left-0 z-[60] w-64 bg-gray-900 shadow-2xl transform ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'} lg:translate-x-0 transition-transform duration-300 border-r border-gray-800`}>
        <div className="flex flex-col h-full">
          ...
          <nav className="flex-1 overflow-y-auto p-4 space-y-1">
            <Link to="/" onClick={handleLinkClick} className="w-full flex items-center space-x-3 p-3 rounded-lg text-white hover:bg-gray-800"><span className="text-xl">🏠</span><span>Home</span></Link>
            <Link to="/casino" onClick={handleLinkClick} className="w-full flex items-center space-x-3 p-3 rounded-lg text-white hover:bg-gray-800"><span className="text-xl">🎰</span><span>Casino</span></Link>
            <Link to="/sports" onClick={handleLinkClick} className="w-full flex items-center space-x-3 p-3 rounded-lg text-white hover:bg-gray-800"><span className="text-xl">🏆</span><span>Sports</span></Link>
            <Link to="/lootboxes" onClick={handleLinkClick} className="w-full flex items-center space-x-3 p-3 rounded-lg text-white hover:bg-gray-800"><span className="text-xl">🎁</span><span>Lootboxes</span></Link>
            ...
            <div> {/* Games Submenu */}
              ...
              {activeSubmenu === 'games' && (<div className="pl-10 pt-2 space-y-2">
                  <Link to="/slots" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">🎰</span><span>Slots</span></Link>
                  <Link to="/table-games" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">🃏</span><span>Table Games</span></Link>
                  <Link to="/roulette" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">🎲</span><span>Roulette</span></Link>
                  <Link to="/blackjack" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">🂡</span><span>Blackjack</span></Link>
                  <Link to="/live-casino" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">👥</span><span>Live Casino</span></Link>
                  <Link to="/jackpots" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">💰</span><span>Jackpots</span></Link>
              </div>)}
            </div>
            
            <Link to="/promotions" onClick={handleLinkClick} className="w-full flex items-center space-x-3 p-3 rounded-lg text-white hover:bg-gray-800"><span className="text-xl">🎁</span><span>Promotions</span></Link>
            ...
            <div> {/* Wallet Submenu */}
              ...
              {activeSubmenu === 'wallet' && (<div className="pl-10 pt-1 space-y-1">
                  <Link to="/deposit" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white w-full text-left"><span className="text-lg">💳</span><span>Deposit</span></Link>
                  <Link to="/withdraw" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white w-full text-left"><span className="text-lg">↩️</span><span>Withdraw</span></Link>
              </div>)}
            </div>

            <Link to="/tournaments" onClick={handleLinkClick} className="w-full flex items-center space-x-3 p-3 rounded-lg text-white hover:bg-gray-800"><span className="text-xl">🏆</span><span>Tournaments</span></Link>
            <Link to="/earn" onClick={handleLinkClick} className="w-full flex items-center space-x-3 p-3 rounded-lg text-white hover:bg-gray-800"><span className="text-xl">💎</span><span>Earn</span></Link>
            <Link to="/token-dashboard" onClick={handleLinkClick} className="w-full flex items-center space-x-3 p-3 rounded-lg text-white hover:bg-gray-800"><span className="text-xl">📊</span><span>Token Dashboard</span></Link>
            ...
            <div> {/* Support Submenu */}
                ...
                {activeSubmenu === 'support' && (<div className="pl-10 pt-2 space-y-2">
                    <Link to="/help" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">❓</span><span>Help Center</span></Link>
                    <Link to="/contact" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">📧</span><span>Contact Us</span></Link>
                    <Link to="/responsible-gaming" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">🛡️</span><span>Responsible Gaming</span></Link>
                    <Link to="/fairness" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">⚖️</span><span>Fairness</span></Link>
                    <Link to="/terms" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">📄</span><span>Terms of Service</span></Link>
                    <Link to="/privacy" onClick={handleLinkClick} className="flex items-center space-x-2 p-2 text-sm text-gray-300 hover:text-white"><span className="text-lg">🔒</span><span>Privacy Policy</span></Link>
                </div>)}
            </div>

            <Link to="/community" onClick={handleLinkClick} className="w-full flex items-center space-x-3 p-3 rounded-lg text-white hover:bg-gray-800"><span className="text-xl">👥</span><span>Community</span></Link>
            <Link to="/profile" onClick={handleLinkClick} className="w-full flex items-center space-x-3 p-3 rounded-lg text-white hover:bg-gray-800"><span className="text-xl">👤</span><span>Profile</span></Link>
          </nav>
      ...
      <nav className="sticky top-0 z-50 bg-black/20 backdrop-blur-md border-b border-[#3C1A4F]/20">
            ...
            <div className="flex items-center justify-between h-16">
                ...
                {/* RESTORED: Main Navigation Links for Desktop */}
                ...
                <div className="flex items-center space-x-4">
                  {!isAuthenticated ? (
                      <button onClick={() => openModal('login')} className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-[#3C1A4F] to-[#36CFC9] text-white rounded-lg font-semibold">
                      ...
                      <div className="flex items-center space-x-4 sm:space-x-8">
                          ...
                          <button onClick={() => openModal('deposit')} className="flex items-center space-x-2 px-4 py-2 bg-gradient-to-r from-[#3C1A4F] to-[#36CFC9] text-white rounded-lg font-semibold">
